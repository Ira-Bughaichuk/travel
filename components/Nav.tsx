...
export default function Nav({open}:INavProps) {
  ...
  return(
    ...
    {navLinks.map((item)=> (<Link key={item.label} href={item.href} className='flex maxMobile:justify-center tablet:inline-block hover:text-secondary duration-500'>{item.label}</Link>))}
    </nav>
  )
}
